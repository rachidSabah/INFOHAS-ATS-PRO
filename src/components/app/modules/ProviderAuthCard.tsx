    ...
    <Card className={`overflow-hidden border-2 transition-colors ${isConnected ? "border-green-500/30" : "border-border"}`}>
      ...
      <CardContent className="space-y-4 pt-0">
        ...
        <div className="flex gap-2 pt-2 border-t border-border">
          {!isConnected && !showGoogleKeyPrompt ? (
            <>
              {/* Google OAuth button (for Z.ai) */}
              {onLoginWithGoogle && (
                <Button
                  onClick={handleGoogleLogin}
                  disabled={loading !== null}
                  className="gap-2 flex-1 bg-white hover:bg-gray-50 text-gray-700 border border-gray-300 shadow-sm dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200 dark:border-gray-600"
                >
                  {loading === "google" ? (
                    <>
