  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    if (event.data?.type === "antigravity-auth") {
      if (event.data.status === "success") {
        // Save tokens via the provider (fire-and-forget — don't await)
        const provider = getAntigravityProvider();
        provider.login(event.data.accessToken).catch(() => {});
        if (event.data.refreshToken) {
          provider.saveRefreshToken(event.data.refreshToken, event.data.expiresIn || 3600).catch(() => {});
        }
        setState("authorized");
