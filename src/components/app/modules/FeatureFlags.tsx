...
const DEFAULT_SEED_FLAGS: Record<keyof Flags, boolean> = {
  ...
  // STRICT opt-in (zen-egress.ts === true): only enable after the relay URL
  // is verified serving /zen (see vercel-relay/README.md).
...
const FLAGS: { key: keyof Flags; label: string; desc: string; icon: string; severity: "safe" | "feature" | "danger" }[] = [
  ...
  { key: "enableZenQuotaGrace", label: "Zen Shared-Egress Grace", desc: "OpenCode Zen failures caused by Cloudflare's SHARED egress IPs no longer mark the provider unhealthy: 429s (its free limiter is keyed per requester IP — a 429 proves the upstream answers) AND Cloudflare WAF challenges / 1015 / 1020 / 52x edge errors (the provider's own CF zone reacting to the shared pool — never a real auth failure). Routing cooldowns still apply; real 401/404/5xx still follow normal rules. See docs/ZEN_SHARED_EGRESS_HEALTH.md for the relay escape hatch.", icon: "HeartPulse", severity: "safe" },
  { key: "zenRelayEnabled", label: "Zen Vercel Relay", desc: "STRICT OPT-IN — verify the relay first (curl https://ats-zen-relay.vercel.app/zen/v1/models must return the model list). When on, all OpenCode Zen traffic routes through the managed Vercel Edge relay (built from vercel-relay/), which egresses from Vercel's AWS IP pool instead of Cloudflare's shared edge pool — a second, independent per-IP quota bucket that overpasses the shared-IP rate limiting. Session headers, quota grace and the free-model registry follow automatically.", icon: "Waypoints", severity: "safe" },
