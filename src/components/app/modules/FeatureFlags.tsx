...
const FLAGS: { key: keyof Flags; label: string; desc: string; icon: string; severity: "safe" | "feature" | "danger" }[] = [
  ...
  { key: "zenRelayEnabled", label: "Zen Vercel Relay", desc: "Route all OpenCode Zen traffic through the managed Vercel Edge relay (ats-zen-relay.vercel.app, built from vercel-relay/). The relay egresses from Vercel's AWS IP pool instead of Cloudflare's shared edge pool — a second, independent per-IP quota bucket that overpasses the shared-IP rate limiting. Session headers, quota grace and the free-model registry follow automatically. Disable to egress directly from opencode.ai again.", icon: "Waypoints", severity: "safe" },
