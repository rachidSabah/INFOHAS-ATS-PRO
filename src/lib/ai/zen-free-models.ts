...
/**
 * MANAGED RELAY RETIRED (2026-09-11): the Vercel-hosted Zen relay
 * (ats-zen-relay.vercel.app, built from vercel-relay/) is gone — the project
 * is now Cloudflare-only. The supported egress escape hatch from Cloudflare's
 * SHARED IP pool is a SELF-HOSTED relay: point the provider's baseUrl at any
 * non-Cloudflare host serving the /zen/* namespace (e.g.
 * https://relay.example.com/zen/v1). Detection keys off the /zen path prefix
 * (isZenChatUpstream), so session headers, quota-grace health and the
 * free-model registry all follow automatically. See
