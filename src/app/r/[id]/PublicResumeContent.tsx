...
/**
 * Heavy body of the public resume reader, split out of page.tsx so the EDGE
 * SSR bundle stays tiny: this tree drags in the whole store + A4 renderer
 * (~1.5MB server-side). page.tsx loads it with next/dynamic ssr:false, which
 * keeps the /r/[id] Edge Function under Vercel Hobby's 1MB cap while
 * next-on-pages (Cloudflare) still sees the required `runtime = "edge"` on
 * the route. Client-only rendering is unchanged: the previous SSR pass also
 * produced just the loading spinner, with data fetched in useEffect.
 *
 * Public resume reader — three resolution paths, in order:
 *  1. Local store lookup (legacy same-device /r/<resumeId> links).
 *  2. Cloud share snapshot — /r/<shareToken> → GET /api/public/shares/:token.
 ...
 */
export default function PublicResumeContent() {
